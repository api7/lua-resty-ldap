# Hand-built RFC 4511 message shapes. https://www.rfc-editor.org/rfc/rfc4511.txt
...
--- config
    location /t {
        content_by_lua_block {
            ...
            local res = assert(protocol.decode_message(ldap_hex("30 0c 02 01 01 61 07 0a 01 00 04 00 04 00")))
...
--- config
    location /t {
        content_by_lua_block {
            local protocol = require("resty.ldap.protocol")
            local ldap_hex = require("ldap_hex")
            local pkt = ldap_hex([[30 49 02 01 02 64 44
...
--- config
    location /t {
        content_by_lua_block {
            ...
            local res2 = assert(protocol.decode_message(ldap_hex("30 0a 02 01 03 64 05 04 01 78 30 00")))
...
--- config
    location /t {
        content_by_lua_block {
            local protocol = require("resty.ldap.protocol")
            local ldap_hex = require("ldap_hex")
            local res = assert(protocol.decode_message(ldap_hex("30 0f 02 01 02 73 0a 04 08 6c 64 61 70 3a 2f 2f 78")))
...
--- config
    location /t {
        content_by_lua_block {
            local protocol = require("resty.ldap.protocol")
            local ldap_hex = require("ldap_hex")
            local res = assert(protocol.decode_message(ldap_hex("30 0c 02 01 02 65 07 0a 01 20 04 00 04 00")))
            ...
            local bad, err = protocol.decode_message(ldap_hex("04 01 41"))
...
--- config
    location /t {
        content_by_lua_block {
            ...
            local bad, err = protocol.decode_message(ldap_hex("30 0d 02 01 01 61 02 0a 02 01 00 04 00 04 00"))
            ...
            local bad2, err2 = protocol.decode_message(ldap_hex("30 0f 02 01 02 73 02 04 08 6c 64 61 70 3a 2f 2f 78"))
